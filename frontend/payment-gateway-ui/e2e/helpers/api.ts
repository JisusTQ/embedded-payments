...
export async function registerMerchant(request: APIRequestContext, prefix = 'merchant'): Promise<MerchantSession> {
  ...
  return { email, password, merchantId: body.merchantId, token, authHeader: { Authorization: `Bearer ${token}` } }
}
...
export async function createIntent(
  ...
  return request.post(`${API_BASE}/api/v1/admin/payments/intents`, {
...
export async function listTransactions(request: APIRequestContext, token: string, query = '') {
  return request.get(`${API_BASE}/api/v1/transactions${query}`, { headers: { Authorization: `Bearer ${token}` } })
}

export async function listRefunds(request: APIRequestContext, token: string) {
  return request.get(`${API_BASE}/api/v1/refunds`, { headers: { Authorization: `Bearer ${token}` } })
}

export async function getTransaction(request: APIRequestContext, token: string, id: string) {
  return request.get(`${API_BASE}/api/v1/transactions/${id}`, { headers: { Authorization: `Bearer ${token}` } })
}
